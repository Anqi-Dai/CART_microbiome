...
pheno_combi <- tibble(pheno_file = list.files('../data/lefse_input', pattern = 'Disease_Class.txt$', full.names = T)) %>% 
  mutate(combi = str_extract(pheno_file, 'pheno.+\\.')) 

pheno_combi %>% 
  mutate(cat_cmd = str_glue('cat {pheno_file} ../data/metaphlan2_result/merged_abundance_table_species_reordered.tsv > ../data/lefse_input/spp_abundace_{combi}tsv')) %>% 
  select(cat_cmd) %>% 
