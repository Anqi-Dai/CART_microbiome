...
bc %>%
  ...
  inner_join(all_pheno) %>% 
  distinct() %>% 
  ggscatter(x = 'V1', y = 'V2', color =  'center') +
  labs(title = 'PCOA of healthy and CART patients center') +
  #theme_void() +
  ggsave('../figs/PCOA(bray-curtis) of healthy and CART patients centetr.pdf')
...
mp %>% 
  ggscatter(x = 'V1', y = 'V2', color =  'pool', shape = 'center') +
  labs(title = 'PCOA of healthy and CART patients') +
  #theme_void() +
  ggsave('../figs/PCOA(bray-curtis) (ASV level)of CART patients_pool.pdf', width = 5, height = 6)
``` 
