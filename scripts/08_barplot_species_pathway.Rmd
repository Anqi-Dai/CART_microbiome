...
cmd_barplot <- read_csv('../data/unique_sig_pathway_with_clean_pwid.csv') %>% 
  select(pwid) %>% 
  mutate(barcmd = str_glue('humann2_barplot  --input ../data/humann2_res/humann2_final_out/humann2_pathabundance_relab_stratified_reordered_with_tox.tsv --focal-feature {pwid} --sort similarity sum metadata --focal-metadatum Toxicity --last-metadatum Toxicity --scaling pseudolog --dimensions 10 5 -o ../figs/barplot_humann2/{pwid}_pw.png')) %>% 
  select(barcmd) 

cmd_barplot %>% 
  write.table('humann2_bar_cmd.sh', quote = F, col.names = F, row.names = F )
```

```{bash}
bash humann2_bar_cmd.sh
```
