...
Now we have 3 subclasses and together with the 2 classes of 35 that gonna make a total combination of 6
...
relab <- read_tsv('../data/humann2_res/humann2_final_out/humann2_pathabundance_relab_unstratified.tsv') %>% 
...
# write out the table
relab_or %>% 
  write_tsv('../data/pheno_aligned_relab_table/humann2_pathabundance_relab_unstratified.tsv')
```
...
pheno_combi <- data_frame(pheno_file = list.files('../data/lefse_input', pattern = '.txt$', full.names = T)) %>% 
  mutate(combi = str_extract(pheno_file, 'pheno.+\\.')) 

pheno_combi %>% 
  mutate(cat_cmd = str_glue('cat {pheno_file} ../data/pheno_aligned_relab_table/humann2_pathabundance_relab_unstratified.tsv > ../data/lefse_input/pathway_abundace_{combi}tsv')) %>% 
  select(cat_cmd) %>% 
  write_csv('cat_combi_6.sh', col_names = F)
```
...
pheno_combi %>% 
  ...
  select(fmt_cmd) %>% 
  write_csv('format_input_6.sh', col_names = F)
```
...
pheno_combi %>% 
  ...
  select(run_cmd) %>% 
  write_csv('run_lefse.sh', col_names = F)
```
...
pheno_combi %>% 
  ...
  select(cmd) %>% 
  write_csv('plotting_de_feature.sh', col_names = F)
```
...
```{bash}
# run in the cmd line THIS HAS TO BE DONE IN THE CMD SINCE RSTUDIO DOES NOT RECOGNIZE THE LEFSE SCRIPT
bash format_input_6.sh
```
