...
# now reduce the abundance to 9 rows, the top 8 and "other"
...
siglefse <- read_tsv('../data/humann2_res/humann2_final_out/humann2_pathabundance_relab_stratified_reordered.tsv') %>% 
  filter(str_detect(pathway,  paste0(pw$pwid, collapse = '|'))) %>% 
  separate(X1, into = c('pwid','taxa'), sep = '\\|')
