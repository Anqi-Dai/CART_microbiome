...
sigpw <- read_csv('../data/unique_sig_PATHWAYID.csv') %>% 
  # grab the pw id which is gonna be able to match the rownames in the stratified table
  mutate(pwid = str_extract(PATHWAYID, '^.+_PWY|^PWY_\\d{3,4}'))  %>% 
  mutate(pwid = str_replace_all(pwid, '_', '-'))

# find how many species total it has in all of the lefse sig PATHWAYIDs
siglefse <- stratified %>% 
  filter(str_detect(PATHWAYID,  paste0(sigpw$pwid, collapse = '|'))) %>% 
  separate(PATHWAYID, into = c('pwid','taxa'), sep = '\\|')
...
```{r}
species_compo_barplot <- function(PHENOVAR, PATHWAYID) {
  # TWO INPUTS: 
  ...
  ggplot() +
    ...
    labs(y = 'relative abundance',
         x = 'Sample',
         title = str_glue('Response to Toxicity NO and YES:\nspecies composition of the pathway abundance\n{PATHWAYID}')) +
    theme_light() +
    theme(
...
sig_num$pwid %>% 
  map(~  species_compo_barplot('Toxicity', .))
