...
res_top <- pmap(ru_top_samp, function(Akkermansia, Bacteroides, Enterococcus, Faecalibacterium, Ruminococcus){
  per_pt_sample_post_prob(Akkermansia, Bacteroides, Enterococcus, Faecalibacterium, Ruminococcus)
}) %>% 
  set_names(paste('P', seq(1, N), sep = '')) %>% 
  bind_rows(.id = 'ptsamp')

# for the bottom quantile patients
res_bot <- pmap(ru_bot_samp, function(Akkermansia, Bacteroides, Enterococcus, Faecalibacterium, Ruminococcus){
  per_pt_sample_post_prob(Akkermansia, Bacteroides, Enterococcus, Faecalibacterium, Ruminococcus)
}) %>% 
  set_names(paste('P', seq(1, N), sep = '')) %>% 
  bind_rows(.id = 'ptsamp')
...
res_top %>% 
  gghistogram(x = 'value', fill = 'ptsamp', add_density = T,
