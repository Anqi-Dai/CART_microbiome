...
```{r}
taxa <- read_tsv('../data/shotgun_output/metaphlan/merged_abundance_table.txt', skip = 1)  %>% 
...
cmds <- tibble(
  ...
  mutate(run_cmd = str_glue('run_lefse.py {fns}.in  {fns}.res')) %>% 
  mutate(plot_cmd = str_glue('plot_res.py {fns}.res {fns}.png  --feature_font_size 4 --width 10 --dpi 300')) %>% 
  mutate(clado_cmd = str_glue('plot_cladogram.py {fns}.res {fns}_clado.png  --label_font_size 4 --dpi 300 --format png --title {fns}')) %>% 
  select(-fns) %>% 
  gather() %>% 
  select(value) %>% 
